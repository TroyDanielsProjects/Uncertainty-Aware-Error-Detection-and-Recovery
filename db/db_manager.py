...
class DBManager:
    ...
    """
    def __init__(self, db_path='db/results.sqlite'):
    ...
    def register_model(self, model_name, architecture=None, training_method=None, mech_config=None):
        conn = self.get_connection()
        if not conn: return None
        cursor = conn.cursor()
        try:
            # Insert if not exists (based on UNIQUE constraint)
            cursor.execute(
    ...
    def create_experiment(self, experiment_name, dataset_name, config, repo_version="1.0"):
        conn = self.get_connection()
        if not conn: return None
        cursor = conn.cursor()
        try:
    ...
    def log_result(self, experiment_id, model_id, question_data, prediction_data, uq_vector, trace_path=None):
        conn = self.get_connection()
        if not conn: return None
        cursor = conn.cursor()
        try:
            ...
            """, (
                ...
                question_data.get('id_external'), question_data.get('question'), question_data.get('gold_answer'),
                prediction_data.get('predicted_answer'), prediction_data.get('is_correct'), prediction_data.get('full_text'),
                trace_path,
                # Map HUV components
                uq_vector.mechanistic_score, uq_vector.avg_entropy, uq_vector.min_logit_gap, uq_vector.saup_score,
                uq_vector.semantic_divergence, uq_vector.semantic_entropy, uq_vector.heuristic_score
            ))
            conn.commit()
            return cursor.lastrowid
        except sqlite3.Error as e:
    ...
    def log_stage_results(self, result_id, stage_data: List[Dict]):
        conn = self.get_connection()
        if not conn: return
        cursor = conn.cursor()
        try:
            ...
            for i, stage in enumerate(stage_data):
                rows.append((
                    result_id, i, stage.get('stage_name'), stage.get('token_count'),
                    stage.get('avg_entropy'), stage.get('min_logit_gap'), stage.get('saup_score')
                ))
...
if __name__ == '__main__':
    # Initialize DB if run directly (for testing)
    dbm = DBManager(db_path=os.path.join(os.path.dirname(__file__), 'results.sqlite'))
    print("Database initialized (if needed).")
