...
class DBManager:
    ...
    def _init_db(self):
        ...
        CREATE TABLE IF NOT EXISTS Results (
            ...
            uq_mech_trace TEXT, uq_entropy_trace TEXT, uq_logit_gap_trace TEXT, uq_tokens TEXT, gen_len INTEGER
        );
        """
        try:
            with self.conn:
        ...
        with self.conn:
            self.conn.execute("""
                INSERT INTO Results (
                    ...
                    uq_mech_score, uq_mech_trace, uq_entropy_trace, uq_logit_gap_trace, uq_tokens, gen_len
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ...
                json.dumps(uq.tokens), # <--- SAVE TOKENS
                uq.num_tokens
            ))
